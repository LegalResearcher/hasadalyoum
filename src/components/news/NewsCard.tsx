      ...
      <Link to={`/article/${post.slug}`} className="block group">
        <div className="bg-card rounded-lg p-4 hover:shadow-lg transition-shadow">
          <div className="flex items-center gap-4 mb-3">
            ...
            <div>
              <p className="font-bold text-foreground">{post.author?.name || "كاتب"}</p>
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 py-2 pl-2">
          {post.category && (
            <span className="text-xs text-accent font-medium">{post.category.name}</span>
          )}
          <h4 className="text-sm font-bold text-foreground line-clamp-2 mt-1 group-hover:text-accent transition-colors">
            {post.title}
          </h4>
          <span className="text-xs text-muted-foreground mt-1 block">{formatDate(post.published_at)}</span>
        </div>
      </Link>
    );
  }
    ...
    return (
      <Link to={`/article/${post.slug}`} className="block group">
        <div className="relative aspect-video rounded-lg overflow-hidden mb-2">
          ...
          />
          {post.category && (
            <span className="absolute top-2 right-2 bg-accent text-accent-foreground px-2 py-0.5 rounded text-xs font-medium">
              {post.category.name}
            </span>
          )}
        </div>
  ...
  return (
    ...
    >
      <div className="relative aspect-video overflow-hidden">
        ...
        />
        {post.category && (
          <span className="absolute top-3 right-3 bg-accent text-accent-foreground px-3 py-1 rounded text-sm font-medium">
            {post.category.name}
          </span>
        )}
      </div>
      <div className="p-4">
        ...
        <span className="text-xs text-dateColor mt-2 block">{formatDate(post.published_at)}</span>
      </div>
    </Link>
  );
};
